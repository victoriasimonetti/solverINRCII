...
#define OGSI CustomOsiGlpkSolverInterface
...
{ if (err != 0)
  { char s[100] ;
    ...
    std::cout
      << "ERROR: " << s
      << " (" << osimethod << " in CustomOsiGlpkSolverInterface)" << std::endl ;
    throw CoinError(s,osimethod.c_str(),"CustomOsiGlpkSolverInterface") ; }
...
#   ifdef GLPK_HAS_INTOPT
    err = lpx_intopt(model,&parm_) ;
#   else
    if (lpx_get_status(model) != LPX_OPT)
    { initialSolve() ; }
    //err = lpx_integer(model) ;
    err = glp_intopt(model,&parm_) ;
#   endif
/*
  ...
  closely.  GLP_IOPT indicates a proven optimal integer solution.
...
/*
  ...
  the bounds.
*/
  if (probStatus == LPX_UNDEF || probStatus == GLP_UNDEF)
  { getColLower() ;
    int j ;
    for (j = 0 ; j < numcols ; j++)
    { colsol_[j] = 0.0 ;
...
{
	// ??? not yet implemented.
	throw CoinError("method is not yet implemented", "getDualRays", "CustomOsiGlpkSolverInterface");
...
{
	// ??? not yet implemented.
	throw CoinError("method is not yet implemented", "getPrimalRays", "CustomOsiGlpkSolverInterface");
	return std::vector<double*>();
}
...
OGSI::CustomOsiGlpkSolverInterface ()
: 
OsiSolverInterface()
{
	gutsOfConstructor();
...
# endif
  if (copyData)
    return( new CustomOsiGlpkSolverInterface( *this ) );
  else
    return( new CustomOsiGlpkSolverInterface() );
}
...
OGSI::CustomOsiGlpkSolverInterface( const CustomOsiGlpkSolverInterface & source )
: 
...
CustomOsiGlpkSolverInterface& OGSI::operator=( const CustomOsiGlpkSolverInterface& rhs )
{
	if( this != &rhs )
	{
...
void OGSI::gutsOfCopy (const CustomOsiGlpkSolverInterface &source)
...
void OGSI::gutsOfConstructor()
{
  ...
  // Init control parameters structure
  glp_init_iocp(&parm_);
...
unsigned int CustomOsiGlpkSolverInterface::numInstances_ = 0;

void CustomOsiGlpkSolverInterface::decrementInstanceCounter()
{
...
glp_iocp* CustomOsiGlpkSolverInterface::getEnvPtr() const {
   return &parm_;
}
