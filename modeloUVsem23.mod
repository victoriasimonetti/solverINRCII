...
s.t. S2aux1{n in N, (u,v) in B, i in u..v: v-u<6}:
	bd[n,u,v] <= sum{s in SD, k in K} x[n,i,s,k];

s.t. S2aux2{n in N, (u,v) in B: u>1 and  v-u<6}:
	bd[n,u,v] + sum{s in SD, k in K} x[n,u-1,s,k] <=1;

s.t. S2aux3{n in N, (u,v) in B: v < card(D) and v-u<6}:
...
s.t. S2a{n in N, (u,v) in B: v-u<6 and v!=card(D)}: # S2a: mínimo de dias consecutivos trabalhados
...
s.t. S2S3aux4{n in N, s in S, (u,v) in B, i in u..v: v-u<6}:
	bs[n,u,v,s] <= sum{k in K} x[n,i,s,k];

s.t. S2S3aux5{n in N, s in S, (u,v) in B: u>1 and v-u<6}:
	bs[n,u,v,s] + sum{k in K} x[n,u-1,s,k] <=1;

s.t. S2S3aux6{n in N, s in S, (u,v) in B: v < card(D) and v-u<6}:
...
s.t. S2c{n in N, (u,v) in B, s in S: v-u<6 and v!=card(D)}:
