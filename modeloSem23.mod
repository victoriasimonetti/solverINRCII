...
minimize obj: wS1 * sum{d in D, s in SD, k in K} CS1[d,s,k] + 
			  ...
			  wS2ab * sum{n in N, (i,j) in B} ( CS2a[n,i,j]) + # min bloco dias consec trab

			  wS2cd * sum{n in N, (i,j) in B, s in SD} ( CS2cS3a[n,i,j,s] ) + # min blocos turnos consec trab s/ folga
			  wS2cd * sum{n in N, s in SD}	CS2cS3ahist[n,s]+ # min blocos turnos s/ folga (historico)

			  wS3 * sum{n in N, (i,j) in B, s in S: s=card(S)} ( CS2cS3a[n,i,j,s]) + # min blocos folgas consec trab 
...
s.t.minConsecdias:
	MinconsecDias30 = sum{n in N, (i,j) in B} ( CS2a[n,i,j]) + sum{n in N, d in D} (ad[n,d] + cd[n,d]) + sum{n in N} (ad[n,0] + cd[n,0]) + sum{n in N, (i,j) in B} ( CS2a[n,i,j]) ;
...
s.t. minConsecTurnos:
	MinconsecTurno15 = sum{n in N, d in D, s in SD} (as[n,d,s] + cs[n,d,s]) + sum{n in N, s in SD}  (as[n,0,s] + cs[n,0,s]) + sum{n in N, (i,j) in B, s in SD} ( CS2cS3a[n,i,j,s]) + sum{n in N, s in SD} CS2cS3ahist[n,s];
...
s.t. minconsecFolga:
	MinconsecFolga30 = sum{n in N, d in D, s in S: s=card(S)} (as[n,d,s] + cs[n,d,s]) + sum{n in N, s in S: s=card(S)}  (as[n,0,s] + cs[n,0,s]) + sum{n in N, (i,j) in B, s in S: s=card(S)} ( CS2cS3a[n,i,j,s]) + sum{n in N, s in S: s=card(S)} CS2cS3ahist[n,s] ;
